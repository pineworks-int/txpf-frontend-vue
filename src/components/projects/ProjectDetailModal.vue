...
const { closeProjectDetailModal } = uiStore
</script>
...
<template>
  <transition name="fade">
    ...
    >
      ...
      >
        <!-- * close button -->
        <button @click="closeProjectDetailModal" class="absolute top-4 right-4 text-gray-500 hover:text-gray-800">
          <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
        </button>

        <!-- * project details -->
        <h1 class="text-3xl font-bold mb-4">{{ selectedProject.title }}</h1>
